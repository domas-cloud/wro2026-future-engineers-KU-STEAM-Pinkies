...
| `VL53L1X` | 2 x `VL53L4CD` | ToF-based obstacle confirmation remains, but the system uses only 2 modules to save power. |
...
- The ToF sensor count is reduced to 2 `VL53L4CD` distance sensors because the camera now contributes more to forward-distance estimation.
...
- the `ESP32` combines the camera information with the `BNO085` and 2 `VL53L4CD` modules;
